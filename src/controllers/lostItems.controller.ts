...
} from "../services/lostItems.service";

/**
 * Retrieves all lost item records.
 */
...
/**
 * Retrieves a single lost item by its ID.
 */
...
export const createLostItem = async (req: any, res: any) => {
  try {
    const requestBody = {
      ...
      room:
        req?.body?.room === A203
          ? $Enums.lost_items_room.A203
          : $Enums.lost_items_room.A204,
    ...
    if (error?.message === `${LOST_ITEM}_${DB_ERROR_CODES.INVALID_DATA}`) {
      res.status(422).send({
        error: `${LOST_ITEM}_${CONTROLLER_ERROR_CODES.MISSING_DATA.message}`,
      });
    } else {
      res
        .status(500)
...
export const updateLostItem = async (req: any, res: any) => {
  ...
  try {
    ...
    if (error?.message === `${LOST_ITEM}_${DB_ERROR_CODES.INVALID_DATA}`) {
      ...
      error?.message === `${LOST_ITEM}_${id}_${DB_ERROR_CODES.NOT_FOUND}`
    ) {
      res.status(404).send({
        error: `${LOST_ITEM}_${id}_${DB_ERROR_CODES.NOT_FOUND}_${CONTROLLER_ERROR_CODES.INVALID_PARAMS.message}`,
      });
