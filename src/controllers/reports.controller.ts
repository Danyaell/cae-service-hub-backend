...
export const createReport = async (req: any, res: any) => {
  try {
    const requestBody = {
      ...
      room:
        req?.body?.room === A203
          ? $Enums.reports_room.A203
          : $Enums.reports_room.A204,
      pc: req?.body?.pc,
    ...
    if (error?.message === `${REPORT}_${DB_ERROR_CODES.INVALID_DATA}`) {
      res
        .status(422)
        .send({
          error: `${REPORT}_${CONTROLLER_ERROR_CODES.MISSING_DATA.message}`,
        });
    } else {
      res
        .status(500)
...
export const updateReport = async (req: any, res: any) => {
  ...
  try {
    ...
    if (error?.message === `${REPORT}_${DB_ERROR_CODES.INVALID_DATA}`) {
      ...
      error?.message === `${REPORT}_${id}_${DB_ERROR_CODES.NOT_FOUND}`
    ) {
      res.status(404).send({
        error: `${REPORT}_${id}_${DB_ERROR_CODES.NOT_FOUND}_${CONTROLLER_ERROR_CODES.INVALID_PARAMS.message}`,
      });
